...
def get_images_for_week(week_str):
    """Get gardencam images for a specific ISO week (e.g., '2026-W08').

    ISO weeks run Monday-Sunday. Converts week to date range and fetches all images.
    """
    if not BOTO3_AVAILABLE:
        return []

    try:
        # Parse ISO week format: "2026-W08"
        year_week = week_str.split(' ')[0]  # Get "2026-W08" part if it has date range appended
        ...
        # January 4th is always in week 1, so use it as reference
        jan4 = datetime(year, 1, 4)
        # Get Monday of week 1
        week1_monday = jan4 - timedelta(days=jan4.weekday())  # weekday(): Mon=0, Sun=6
        # Calculate Monday of target week
        target_monday = week1_monday + timedelta(weeks=week - 1)
        target_sunday = target_monday + timedelta(days=6)

        # Fetch images from all 7 days of the week
        s3 = boto3.client("s3", region_name=GARDENCAM_REGION)
        all_images = []

        for day_offset in range(7):
            current_date = target_monday + timedelta(days=day_offset)
            date_str = current_date.strftime("%Y-%m-%d")
            date_prefix = f"garden_{current_date.strftime('%Y%m%d')}"
        ...
        print(f"Error fetching images for week {week_str}: {e}")
        return []
...
def get_latest_gardencam_images(count=3):
    ...
    for batch_size in batch_sizes:
        ...
        for img in all_images:
            # Display all images (filtering disabled for performance)
            # Fetch stats only for display labels if needed
            stats = get_image_stats_by_filename(img['key'])

            # Use 150px thumbnail for display (created by Lambda on S3 upload)
            # Trust that thumbnails exist - they're created automatically now
            thumb_key = f"thumb_150px_{img['key']}"
...
def lambda_handler(event, context):
    ...
    if path == f'/{stage}/event' or path == '/event':   # debugging info
        ...
        <div class="nav">
            ...
            <a href="/gardencam/gallery">Gallery</a>
            <a href="/gardencam/videos">Videos</a>
        ...
        if image_key:
            ...
            html += '<h1>Error: No image specified</h1>'
    elif path.startswith(f'/{stage}/gardencam/handnail') or path.startswith('/gardencam/handnail'):
        # Handnail view - 800px thumbnail
        if not check_basic_auth(event, GARDENCAM_PASSWORD):
            return {
                'statusCode': 401,
                'body': '<html><body><h1>401 Unauthorized</h1><p>Access denied.</p></body></html>',
        ...
        if image_key:
            timestamp = parse_timestamp_from_key(image_key) or 'Unknown'
            # Use 800px thumbnail
            thumb_800_key = f"thumb_800px_{image_key}"
            image_url = get_presigned_url(thumb_800_key)
        ...
        if not week_param:
            ...
            <div class="nav">
                ...
                <a href="/gardencam">Latest</a>
                <a href="/gardencam/videos">Videos</a>
                <a href="/gardencam/stats">Statistics</a>
            </div>
            ...
            for week_name in weeks:
                ...
                html += f'''
                <a href="/gardencam/gallery?week={week_name}" class="week-link">
                    {week_name}
                </a>
                '''
            ...
            if not current_week_images:
                html += '<h1>Week not found</h1><p><a href="/gardencam/gallery">Back to Gallery Index</a></p>'
            else:
                ...
                <div class="nav">
                    <a href="../../contents">Home</a>
                    <a href="/gardencam/gallery">All Weeks</a>
                    <a href="../gardencam">Latest</a>
                </div>
                <h1>{week_param}</h1>
                <div class="day-list">
                ...
                for day_name, day_images in days:
                    html += f'''
                    <a href="/gardencam/gallery?week={week_param}&day={day_name}" class="day-link">
                        {day_name}
                    </a>
                    '''
            ...
            if not current_day_images:
                html += f'<h1>No images found for {day_param}</h1><p><a href="/gardencam/gallery?week={week_param}">Back to {week_param}</a></p>'
            else:
                ...
                if day_index > 0:
                    ...
                    prev_link = f'<a href="/gardencam/gallery?week={week_param}&day={prev_day}">← Previous Day</a>'
                ...
                <style>
                    ...
                    <div class="nav">
                        <a href="../../contents">Home</a>
                        {prev_link}
                        <a href="/gardencam/gallery?week={week_param}">Week Index</a>
                        <a href="/gardencam/gallery">All Weeks</a>
                        <a href="../gardencam">Latest</a>
                        {next_link}
                    </div>
                ...
                for img in chunked_images:
                    ...
                    # Link to handnail (800px preview)
        ...
        <div class="nav">
            ...
            <a href="/gardencam/gallery">Gallery</a>
            <a href="/gardencam/videos">Videos</a>
            <a href="/gardencam/stats">Capture Stats</a>
        </div>
        ...
        <div class="nav">
            <a href="../../contents">Home</a>
            <a href="../gardencam">Latest</a>
            <a href="/gardencam/videos">All Videos</a>
        ...
        <div class="container">
            ...
            <div class="info-section">
                ...
                <div style="text-align: center; margin-top: 1.5rem;">
                    <a href="/gardencam/videos" class="button">View All Videos</a>
        ...
        try:
            ...
            <div class="error">
                ...
                <p><a href="/gardencam/videos" style="color: #4a9eff;">View all videos</a></p>
            </div>
            '''
        except Exception as e:
